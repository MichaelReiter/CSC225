...
public class DFS{
	...
	*/
	public static void DFS(int[][] G){
		...
		//initialized all to 0's automatically by Java
		int[] visited = new int[n];
		
		rDFS(G, visited, 0);
	}

	private static void rDFS(int[][] G, int[] visited, int curr) {
		System.out.print(curr + " ");
		visited[curr] = 1;

		for (int j = 0; j < visited.length; j++) {
			// if node j is unvisited and there is an edge between current node and j, my a recursive call
			if (visited[j] == 0 && G[curr][j] == 1) {
				rDFS(G, visited, j);
			}
		}
	}
